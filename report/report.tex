...
population from the reconstruction and we force the coherences to 0. The problem
is just on probability vectors on the diagonal. So our input is a set of values
$p_i$ and deviations $\Delta p_i$ such that each $p_i$ marginal law are normal
laws. In theory the probability distribution on $\R^n$ is:
...
iteration of the Markov chain is to start from the previous point $x_{n-1}$ and
sample a random direction in the hyperplane of sum 1. This direction and
$x_{n-1}$ gives us an affine line of that hyperplane.
...
stationary distribution of this Markov chain, and the chain fully mix the set
$\mathcal{P}_n$. On discrete probability sets, This is sufficient to say that there
is only one stationary distribution and that the chain converge toward it. I
hope that this is still true for continuous chain. I decided that this is an inexact
...
If I assume that the chain indeed converge toward the right probability
