...
\chapter*{Abstract}
\addcontentsline{toc}{chapter}{Abstract~--~Notations}
...
In this chapter I expect the reader to know the basics of quantum mechanics, Hilbert space and
...
\begin{figure}[h]
  ...
  \caption{A symbolic representation of the experiment (Credit: Luis Najera).
    ...
    go through in each batch. Most unselected atom
    ...
    At the exit atom are detected by a projective measurement
...
``Rabi oscillation''. The pulsation $\Omega_r$ which is the frequency of
precession when the the frequencies match is named ``Rabi frequency'' and is
...
describes the spacial disctribution of the electric field inside the cavity which depends on the cavity shape.
...
($\sigma_-a^\dagger$) oscillates at the difference frequency $\delta = \omega_a -
\omega_c$. We can
...
energy of state $\ket{g,n}$, the new one will be:
...
we have a shift of phase $\phi_0 = \frac{\Omega_0^2}{2\delta}t\eff$. In
particular, the most controllable parameter here is $\delta$ via the stark effect
on the atom.
...
\TODO Merge
The QND measurement in the current experiment is realized by sending a sequence of individual atoms prepared in a superposition state $\ket{g}+\ket{e}$ through the
cavity and interacting with it dispersively. One can deduce from~\cref{sec:atomcav} that
an atom in this case will undergo a phase shift between its states
proportional to the number of photons stored in the cavity field. By configuring well the
Ramsey zones, one can build a Ramsey interferometer and measure this phase
shift. However the only concrete output we get is a long list of state detections in $\ket e$ and
$\ket g$, so we need a statistical tool to reconstruction a corresponding
density matrix from this data. This what is presented in the next chapter. I
...
Our method of QND measurement works by sending a lots of atoms in a Bell state through the
cavity at an off-resonance setting. As they are in the dispersive setting(see
\cref{sssec:disp}, they
will all undergo a phase shift of $\phi_0$ by photons in the cavity. If
calibrate our system to have an interesting $\phi_0$, the atomic state will be
dispersed on the equator of the block sphere depending on the number of photon
in the cavity. We can then do a $\frac\pi2$ Rabi-pulse at a specific phase to
rotate the equator to a polar circle on the Bloch sphere. When we measure the
atom, the probabilities of being observed in $\ket e$ or $\ket g$ depends on the
number of atoms in the cavities. This setup is called a Ramsey interferometer.
...
However the only concrete output we get is a long list of $\ket e$ and
$\ket g$, so we need a statistical tool to build an representation of the
density matrix from this date.
...
the left. We are forcing the heat to move from the atom to the other cavity.
Then we can plot the mutual information in \TODO{}

\begin{figure}
  \centering
  \includegraphics[height=0.45\textheight]{plots/Heat1.png}
  \caption{Plot of the exchanged heat depending on the Maxwell demon state ($ON$ or $OFF$)}
  \label{fig:heat1}
\end{figure}
...
\begin{figure}
  ...
  \caption{Plot of the exchanged if with the Maxwell demon $ON$ and $OFF$}
  \label{fig:info1}
\end{figure}
...
\addcontentsline{toc}{chapter}{Conclusion~--~Thanks}
