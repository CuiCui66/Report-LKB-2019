...
A observable $A$ is any Hermitian matrix on $\mathcal{H}$. The
...
and as $\bra \Psi I \ket \Psi = \Tr(\rhoI) = 1$, the probabilities are normalized
...
\begin{defn}
  On a quantum Hilbert space $\mathcal{H}$, a generalized measurement is a set
...
\begin{prop}
  ...
  to measure $o$ with probability, $\Tr(M_0\rho M_0^\dagger)$. The outcome would
  then be:
  \[\frac{M_0\rho M_0^\dagger}{\Tr(M_0\rho M_0^\dagger)}\]
\end{prop}



\subsection{Quantum operations, Krauss maps}
