...
%\TODO{}
% , I'll explain why in \cref{ssec:ststab}.
...
use those result with low-regularity function. However those conditions are not
sufficient to deal with the entropy on the edge of $\mathcal{D}$. In the
\cref{sec:regentropy}, I explain how one can achieve similar results with the
entropy but with secondary term have a smaller decreasing compared to the main
term.
...
We make some technical assumtions such as:
\begin{itemize}
\item $P_0(\rho\ml) > 0$ : If our estimator has a null-probability, the prior
  distribution doesn't make much sense.
\item $\mathcal{D} = \mathcal{D}(\C^n)$ : We use simple $n$-dimensional density matrices.
\end{itemize}
...
\begin{proof}
  Same remark as in the two last proofs.
\end{proof}
...
\begin{defn}
  ...
  for $k = \lfloor \beta \rfloor$, the function is $\class k$ and we can write:
  \[f(x) = f(0) + xf^{1}(x) + \ldots + \frac{x^{(k)}}{k!}f^{(k)}(0) +  x^\beta g(x)\]
  with $g \in \class{0}$.
\end{defn}

\begin{prop}
The von-Neumann entropy is of $\class \varepsilon$ for any
$\varepsilon < 1$ on the edge of $\mathcal{D}$. I'll say the entropy is of class $\class{1-}$
\end{prop}

\begin{prop}
The variance of the von-Neumann entropy is of $\class \varepsilon$ for any
$\varepsilon < 2$ on the edge of $\mathcal{D}$.
\end{prop}

I won't have time to do that in this report, but we can then reestablish the
theorem of previous sections and prove that the
entropy has the usual expectation but with a lower constraint:
\[E_S(N) = S(\rho\ml) + O(N^{-\varepsilon})\]
for any $\varepsilon <1$. It is likely that as $\varepsilon \to 1$, the hidden constant
of the $O$ will explode. Normally it suffice to go over all the proofs of
previous sections and everything works seamlessly.
The same kind of result will hold for the variance of the entropy.

I haven't tried them all, but I think all the other entropy-related values like
mutual-information will also be for the most par of $\class {1-}$.
...
\begin{prop}
  ...
  Then, if $c$ is strictly concave, we'll have:

  \[ \rho_\varepsilon \xrightarrow[\varepsilon \to 0]{} \rho\ml = \argmax_{\rho \in M} c\]
\end{prop}

\begin{proof}
  The proof can probably be found in many convex optimization textbooks.
  %  \TODO{}
\end{proof}
