...
\TODO{} Check that the hermitian case is not bullshit.
...
\begin{thm}[KKT conditions]\label{thm:KKT}
  ...
  \item The gradient of the Lagrangian $\dpar L x$ is 0:
    \[ \nabla f_0(x) + \sum_{i = 1}^m \braket{\eta_i}{\nabla f_i(x)} +
      \sum_{j=1}^p \lambda_j h_j(x) = 0\]
  \end{itemize}
...
The optimization is on the vector space $\mathcal{O}(\mathcal{H})$.
